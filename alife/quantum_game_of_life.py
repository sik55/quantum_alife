...
def quantum_2d_cellular_automaton(width=20,
    ...
    env_interaction_theta : float
        寿命パラメータ。環境との相互作用の強さ。0～pi/2 の範囲で設定。（0で影響なし、pi/2で最大）、TODO：ランダムに決定
    death_value : float
        寿命パラメータ。生き残り条件の閾値。寿命（0～1）がこの値を下回ると死となる。
    non_live : list of int
        生存パラメータ。誕生しない条件。周囲で生存している生命の数をリストで指定する。
    live_theta : float
        生存パラメータ。周囲で生存している生命による作用の強さ(rad)。大きいほど新たな生命が誕生しやすい。
    live_low : float
        生存パラメータ。誕生判定の下限閾値（値が大きいと生まれにくい、疎状態の評価）。
    live_high : float
        生存パラメータ。誕生判定の上限閾値（値が小さいと生まれにくい、密状態の評価 ）
    data_file : bool
        ファイルパラメータ。データファイル出力する場合はTrue。
    data_file_name : str
        ファイルパラメータ。データファイルのフルパス（ファイル名含む）。
    data_num : int
        記録するデータの数（繰り返し数）。
    shots : int
